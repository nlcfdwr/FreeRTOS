...
void DHT_GPIO_SET_OUTPUT(void)     //设置GPIOx为输出模式（MCU的IO口向DHT11发激活信号）
{
	...
	GPIO_InitStructure.Pin=GPIO_PIN_9;      //设置的格式必须严格遵循注释，比如GPIO_PIN_define
...
uint8_t DHT_Read_Byte(void)  //从DHT11读取一位（8字节）信号
{
	 ...
	 {
			while(HAL_GPIO_ReadPin(GPIOG,GPIO_PIN_9)==0 && retry<100)  
		  ...
		  //延时30us后，如果IO读取到仍是高电平，说明采集到1；如果IO读取到低电平，说明采集到0
			...
			if(HAL_GPIO_ReadPin(GPIOG,GPIO_PIN_9)==1)   temp=1;
			 else   temp=0;

			 while(HAL_GPIO_ReadPin(GPIOG,GPIO_PIN_9)==1 && retry<100)
...
uint8_t DHT_Read(void)
{
	 ...
	 HAL_GPIO_WritePin(GPIOG,GPIO_PIN_9,GPIO_PIN_RESET);   //IO->DHT11:先拉低电平18ms（应时序要求）
	 //HAL_Delay(18);
	 vTaskDelay(18);
	 HAL_GPIO_WritePin(GPIOG,GPIO_PIN_9,GPIO_PIN_SET);   //IO->DHT11:随后拉高电平20us
	 delay_us(20);
	
	//MCU通过IO向DHT11发送请求完毕。接下来DHT11向IO发送响应,IO转为输入模式。在这之后就开始信号的转译读取。
	DHT_GPIO_SET_INPUT();
	delay_us(20);
	if(HAL_GPIO_ReadPin(GPIOG,GPIO_PIN_9)==0) //DHT11发回低电平响应（读取到低电平，说明DHT11有响应）
	{
