...
void uart_device_init(uint8_t uart_id)
{
	...
	{
		/* 配置串口1收发fifo */
		fifo_register(&s_uart_dev[uart_id].tx_fifo, &s_uart1_tx_buf[0], //为什么注册的发送fifo不需要加锁，而接收的fifo需要加锁呢？
 ...
 * @retval 
 */
uint16_t uart_read(uint8_t uart_id, uint8_t *buf, uint16_t size)//第一个参数是串口1还是串口2，第二个参数是待读取的数据缓存，第三个参数是缓存大小
{
...
void uart_dmarx_half_done_isr(uint8_t uart_id)
{
	...
	if(uart_id == 0)
	{
	  	recv_total_size = s_uart_dev[uart_id].dmarx_buf_size - bsp_uart1_get_dmarx_buf_remain_size();//得到目前 DMA 接收了多少数据
	}
	else if (uart_id == 1)
	{
		//recv_total_size = s_uart_dev[uart_id].dmarx_buf_size - bsp_uart2_get_dmarx_buf_remain_size();
	}
	recv_size = recv_total_size - s_uart_dev[uart_id].last_dmarx_size;//本次中断相比上次中断新收到了多少数据
	s_UartTxRxCount[uart_id*2+1] += recv_size;//记录接收到的数据总量,用于调试或统计
    
	fifo_write(&s_uart_dev[uart_id].rx_fifo, 
				   (const uint8_t *)&(s_uart_dev[uart_id].dmarx_buf[s_uart_dev[uart_id].last_dmarx_size]), recv_size);//将这次新收到的数据写入软件 FIFO
	s_uart_dev[uart_id].last_dmarx_size = recv_total_size;//更新 last_dmarx_size，记录下当前 DMA 已接收的数据位置，以便下一次比较
}
...
void uart_poll_dma_tx(uint8_t uart_id)
{
    ...
    {
        return;//如果当前UART正在进行DMA发送发送，就直接return
    }
	size = fifo_read(&s_uart_dev[uart_id].tx_fifo, s_uart_dev[uart_id].dmatx_buf,
					 s_uart_dev[uart_id].dmatx_buf_size);//从该UART的发送FIFO(tx_fifo)中读取最多dmatx_buf_size个字节的数据
	if (size != 0)
	{	
        s_UartTxRxCount[uart_id*2+0] += size;//记录发送数量
	  	if (uart_id == 0)
		{
